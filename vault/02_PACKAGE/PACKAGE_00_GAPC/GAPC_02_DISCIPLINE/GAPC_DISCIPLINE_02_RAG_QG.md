...
depends_on: [DISCIPLINE_02_RAG_QG, DISCIPLINE_01_GEL_RULES, CONSTRAINT_01_RAG_SCOPE_POLICY, CONSTRAINT_00_GUARD_RAILS, GAPC_DISCIPLINE_00_RAG_PROFILE, GAPC_DISCIPLINE_04_RISK_REGISTER, GAPC_DISCIPLINE_03_SCOPE_CLASSIFIER]
arc: PACKAGE
...
Ces gates **renforcent** les gates CORE (`DISCIPLINE_02_RAG_QG`) et ne les remplacent pas.
...
## 1) Pré-requis
Les critères CORE de `RAG_READY_TO_FREEZE` doivent être PASS (NON TROUVÉ, sources, actifs uniques, no-secrets).
...
- Les réponses ne citent **que** CORE + PACKAGE_00_GAPC + PRODUCT actif (si applicable) + SYSTEM,
...
### 2.4 Traçabilité (P0)
- Toute recommandation structurante propose une ADR-lite (pas de décision implicite).
...
- Aucun risque critique “RAG” GAPC (score 6–9) n’est Open sans mitigation + owner (`GAPC_DISCIPLINE_04_RISK_REGISTER`).

### 3.3 Amendements contrôlés (P0)
- Toute modification des règles GAPC RAG passe par patch + validation + bump version (mécanisme défini par gel GAPC).
...
- v1.0 (01-03-2026) : création gates RAG GAPC (renforcement CORE) : GAPC-only sources + alignement discipline + anti-derives + traçabilité.