...
CORE strict : aucune procédure opérateur (pas de commandes, pas d’evidence pack).
...
- + 1 action pour rendre l’information trouvable (créer/compléter un doc, tracer une ADR).
...
### 1.6 Anti-cross-arc
Un changement ne doit pas toucher plusieurs arcs (CORE+PRODUCT, etc.) sans découpage explicite.
...
PACKAGE/PRODUCT peuvent renforcer ces garde-fous (plus strict), mais ne peuvent pas :
