...
Permettre d’utiliser Git **sans théorie inutile**, dans le framework GAPC (Vault → Repo), en appliquant :
- **1 intention = 1 commit**
- **diff-first** (tu lis avant d’agir)
- **no-secrets** (aucun token/clé/PII versionné)
- workflow compatible avec l’architecture : `00_SYSTEM/01_CORE/02_PACKAGE/03_PRODUCT/04_CACHE`.

> Règle : travailler toujours depuis la racine repo (dossier contenant `.git/`).  
...
- **Rebase/Merge** : méthodes pour intégrer/aligner (débutant : suivre la policy du framework).
...
**Stop condition**
- Si `git status` montre “detached HEAD” ou “merge in progress” → ne pas continuer sans lire §6.
