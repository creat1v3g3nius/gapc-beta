...
- l application quotidienne des obligations de gouvernance passe par `WORKFLOW_03_START_SESSION`, `WORKFLOW_04_END_SESSION` et `WORKFLOW_05_INCIDENT`.
...
## Compatibilite
- un product gele doit tracer les versions de `CORE`, `SYSTEM` et `PACKAGE` sur lesquelles il repose
...
- toute modification de `CORE`, `SYSTEM` ou `PACKAGE` qui affecte un product actif impose une revalidation
- toute modification locale stable dans `PRODUCT` appelle une decision de merge-out

## Obligations operatoires derivees
- tout changement de perimetre actif doit etre explicite des l ouverture de session
- toute chaine de preuve mixte ou ambiguite de perimetre declenche un traitement via `WORKFLOW_05_INCIDENT`
- toute cloture de session doit laisser un next step unique et un etat de product explicite
