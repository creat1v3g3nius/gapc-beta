...
Ce document permet d'exécuter en **30–45 minutes** une session de
production **répétable** et **sans dérive** sur **n’importe quel package**
et **n’importe quel product** :

**start → 1 composant (CO) → patch → validator/smoke → commit → test mentor
(RAG) → clôture**.
...
`WORKFLOW_01_COMPOSANTS` est desormais `DEPRECATED` et ne sert plus de source
active.
...
> Donne-moi la checklist applicable pour exécuter ce CO, et cite les fichiers
  utilisés.
...
- v1.0 (27-02-2026) : version générique (multi-package/multi-product),

  anti-dérive, compatible Obsidian/VS Code/AnythingLLM.
...
- v1.3 (13-03-2026) : retire la dependance a `WORKFLOW_01`, recable les CO

  vers `PIPELINE_03_BACKLOG_COMPOSANTS`.
