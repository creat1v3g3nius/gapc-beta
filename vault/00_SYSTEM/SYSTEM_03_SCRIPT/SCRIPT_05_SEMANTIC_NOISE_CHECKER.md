...
depends_on: [META_01_OUTPUT_PROTOCOL, META_02_SOP_STANDARD_LOOP, META_05_FRONTMATTER, DISCIPLINE_03_DOC_QG, SCRIPT_04_DOC_INTEGRITY_CHECKER, FRAMEWORK_00_AMELIORATION_PIPELINE]
arc: SYSTEM
...
Definir le **process de creation** du script `SemanticNoiseChecker` dans `00_SYSTEM/03_SCRIPT/`.

Ce script controle le **bruit semantique** du corpus documentaire.

Controles cibles :
- variations lexicales d'IDs (`GUARDRAILS` vs `GUARD_RAILS`, `COMPOSANT` vs `COMPOSANTS`)
...
Ce document decrit **les regles**, **le process**, **les gates documentaires** et **les sorties attendues**.
Il ne contient aucun secret, aucune donnee sensible, aucune procedure de deploiement.
...
Un ecart est `P1` s'il n'empeche pas la structure minimale, mais introduit une derive.
...
- confirmer le positionnement : bruit semantique, pas validation structurelle brute
