...
Les regles transverses restent dans `CORE`, les overlays restent dans `PACKAGE_00_GAPC`.
...
- les tests de `DOD_02_RAG_WORKSPACE_TESTS` sont executables
...
PASS si :
- un thin slice documente existe (`DOD_01_PRODUCT_THIN_SLICE`)
- un lot de tests RAG existe (`DOD_02_RAG_WORKSPACE_TESTS`)
- une checklist de gel existe (`DOD_03_RELEASE_FREEZE`)
...
Next step unique:
- consolider ce verdict dans `DOD_03_RELEASE_FREEZE`
```
...
- v1.2 (13-03-2026) : aligne le verdict final sur le lot `OPS` backfill et ajoute la preuve `EVIDENCE_04_CODEX_IDE`.
